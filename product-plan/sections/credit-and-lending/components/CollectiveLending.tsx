...
import {
  ...
  ThumbsUp, ThumbsDown, AlertCircle, Calendar, ChevronDown, ChevronUp
} from 'lucide-react'
import type { CollectiveLoan, CollectiveLoanVote } from '../types'
...
function VoteProgress({ approved, denied, required }: { approved: number; denied: number; required: number }) {
  ...
  const remainingPercent = 100 - approvedPercent - deniedPercent
...
export function CollectiveLending({
  ...
  const completedLoans = collectiveLoans.filter(l => ['completed', 'declined'].includes(l.status))
