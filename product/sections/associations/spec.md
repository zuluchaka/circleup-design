...
- **Discover Associations** — Search/filter by type, language, location, association type (cultural, savings, professional, federation); each card's **Request to join** opens a **Pre-Check modal** (web) / **Pre-Join BottomSheet** (mobile) before the request is submitted:
  - Header summarises the target association (logo, name, country, member/circle counts).
  - **Verdict banner** — auto-approve (all rules clear), needs review (warnings/manual items), or blocking (failed rules).
  - **Eligibility pre-checks** — evaluation of the current user's profile against each of the association's rules (KYC, Trust Score threshold, language, geo/residence, referral, duplicate-membership), with passed / warning / failed / manual states and per-rule detail.
  - **Applicant message** — required textarea where the user introduces themselves; the message is the same field surfaced to the president inside the Join Request Review modal.
  - **Submit / Cancel** — Submit is disabled while any check is `failed`, with a "Resolve issues to send" affordance. Once submitted the request appears in the member's **Pending requests** section on My Associations Dashboard and in the president's **Pending approvals** queue.
...
- **Discover Associations** — Filter chips, association cards with type badge, language, member count, "Request to Join" → opens **Pre-Check modal / sheet** with verdict banner, eligibility pre-check list, applicant-message field, Submit/Cancel.
