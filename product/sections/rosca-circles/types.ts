...
export type CircleStatus = 'forming' | 'active' | 'paused' | 'completed' | 'cancelled'
...
export type PaymentMethodType = 'card' | 'bank_account' | 'sepa_debit' | 'mobile_money' | 'twint'
...
export type CircleAccountStatus = 'active' | 'frozen' | 'closed' | 'pending'
...
export interface ReceivedCircleInvitation {
  id: string
  circleId: string
  circleName: string
...
// =============================================================================
// Circle Account (B2B Fund Wrapper) Types
// =============================================================================
...
// =============================================================================
// Cycle Progress Types (CO-US012)
// =============================================================================

export interface CyclePaymentGridEntry {
  memberId: string
  memberName: string
...
export interface CycleProgressData {
  ...
  paymentGrid: CyclePaymentGridEntry[]
}

// =============================================================================
// Circle Health Analytics Types (CO-US014)
// =============================================================================
...
export interface CircleHealthMetrics {
  ...
  memberCount: number
  collectionRate: number
  collectionRateTrend: string
  onTimePaymentRate: number
  onTimePaymentTrend: string
  totalCollected: number
  totalDisbursed: number
  emergencyFundBalance: number
  ...
  benchmarkComparison: {
    collectionRate: { value: number; benchmark: number; status: string }
    onTimePayment: { value: number; benchmark: number; status: string }
    engagement: { value: number; benchmark: number; status: string }
  }
  monthlyTrends: Array<{
    cycle: number
    collectionRate: number
...
// =============================================================================
// Circle Renewal Types (CO-US015)
// =============================================================================

export interface RenewalVoteSummary {
  optIn: number
  optOut: number
  pending: number
...
export interface CircleRenewalVote {
  userId: string
  userName: string
  vote: 'opt_in' | 'opt_out'
  reason: string | null
...
export interface CircleRenewal {
  id: string
  circleId: string
  status: RenewalStatus
  proposedStartDate: string
  newContributionAmount: number | null
  newDurationMonths: number | null
  newMaxMembers: number | null
  newFrequency: string | null
  notes: string | null
  initiatedBy: { id: string; name: string }
  voteSummary: RenewalVoteSummary
  votes: CircleRenewalVote[]
  newCircleId: string | null
...
// =============================================================================
// Payout Advance & Multi-Share Types
// =============================================================================
...
export interface PayoutAdvance {
  ...
  status: 'requested' | 'approved' | 'denied' | 'disbursed' | 'repaid'
  reason: string
  feePercent: number
  feeAmount: number
  requestedAt: string
...
export interface ApplicantRiskAssessment {
  userId: string
  userName: string
  riskScore: number
  riskLevel: RiskLevel
...
export interface MemberRiskScoresProps {
  ...
  onAssessAll?: () => Promise<unknown>
  onAssessApplicant?: (userId: string) => Promise<ApplicantRiskAssessment | null>
}
...
export interface InviteMembersProps {
  circle: Circle
  invitations: CircleInvitation[]
  onSendInvitation?: (email: string, phone: string | undefined, channel: InvitationChannel, message?: string) => void
...
/** Props for the Cycle Progress view (CO-US012) */
export interface CycleProgressProps {
  data: CycleProgressData
  circle: Circle
  isOrganizer: boolean
  onTriggerPayout?: () => void
  onRetryPayment?: (memberId: string) => void
}

/** Props for the Circle Analytics Dashboard (CO-US014) */
export interface CircleAnalyticsDashboardProps {
...
/** Props for the Circle Renewal Panel (CO-US015) */
export interface CircleRenewalProps {
  circle: Circle
  renewals: CircleRenewal[]
  isOrganizer: boolean
  currentUserId: string
  onPropose?: (form: { proposedStartDate: string; newContributionAmount?: number; newDurationMonths?: number; notes?: string }) => void
  onVote?: (renewalId: string, vote: 'opt_in' | 'opt_out', reason?: string) => void
  onStartVoting?: (renewalId: string) => void
  onCreateCircle?: (renewalId: string) => void
  onCancel?: (renewalId: string) => void
}

/** Props for Circle Settings */
...
/** Props for the Circle Account (B2B) Dashboard */
export interface CircleAccountDashboardProps {
  dashboard: AccountDashboard
  onBack?: () => void
  onExport?: (format: 'pdf' | 'csv') => void
}

/** Props for the Admin Circle Monitoring view (Platform Staff) */
export interface AdminCircleMonitoringProps {
  circles: Array<{
    circle: Circle
    healthScore: number
    openDisputes: number
  ...
  onViewCircle?: (id: string) => void
  onFreezeAccount?: (circleId: string, reason: string) => void
  onFlagCircle?: (circleId: string, reason: string) => void
}

/** Props for the Circle Participants standalone list */
export interface CircleParticipantsProps {
  circle: Circle
  participants: Participant[]
  currentUserId: string
  isOrganizer: boolean
