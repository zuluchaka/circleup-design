...
import type { InviteMembersProps, InvitationChannel, InvitationStatus } from '@/../product/sections/rosca-circles/types'
...
export function InviteMembers({
  ...
  const addEmail = (raw: string) => {
    ...
    if (unique.length > 0) setEmails(prev => [...prev, ...unique])
  }
    ...
    try {
      if (channel === 'email') {
        ...
        for (const addr of allEmails) {
          onSendInvitation(addr, undefined, channel, message || undefined)
        }
        setEmails([])
        setEmailInput('')
      } else if (phone) {
        onSendInvitation('', phone, channel, message || undefined)
        setPhone('')
      }
      setMessage('')
    } finally {
  ...
  const hasRecipient = channel === 'email' ? (emails.length > 0 || (emailInput.trim() && emailInput.includes('@'))) : !!phone
          ...
          <div className="space-y-4">
            ...
            {channel === 'email' && (
              <div>
                ...
                <div className="w-full min-h-[42px] px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 focus-within:ring-2 focus-within:ring-indigo-500 flex flex-wrap gap-1.5 items-center">
                  {emails.map((addr) => (
                    <span
                      key={addr}
                  ...
                  <input
                    ...
                    placeholder={emails.length === 0 ? 'Enter emails separated by commas, spaces, or press Enter' : ''}
                    className="flex-1 min-w-[200px] bg-transparent outline-none text-slate-900 dark:text-white placeholder-slate-500 text-sm py-0.5"
                  />
                </div>
            ...
            >
              {sending ? 'Sending...' : channel === 'email' && (emails.length + (emailInput.trim() && emailInput.includes('@') ? 1 : 0)) > 1
                ? `Send ${emails.length + (emailInput.trim() && emailInput.includes('@') ? 1 : 0)} Invitations`
                : 'Send Invitation'}
            </button>
          </div>
        </div>
