...
export function CreateCircleWizard({
  ...
  /** Initial step synced from URL (e.g. '0', '1', ...) */
  ...
  const resolvedInitial = (() => {
    if (initialStep != null) {
      ...
      if (!isNaN(idx) && idx >= 0 && idx < steps.length) return steps[idx].id
    }
    return 'basics' as WizardStep
  })()
  const [currentStep, setCurrentStep] = useState<WizardStep>(resolvedInitial)
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <div className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sm:hidden">
        <WizardProgressDots
          totalSteps={steps.length}
          currentStep={currentStepIndex}
          labels={steps.map((s) => s.label)}
          onStepClick={(i) => { if (i <= currentStepIndex) setCurrentStep(steps[i].id) }}
        />
      </div>

      {/* Desktop Progress Steps */}
      <div className="hidden sm:block bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      updateFormData({ duration: val, maxParticipants: val })
                    }}
                    min={2}
                    max={52}
                    className="w-full px-4 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                ...
                <div>
                  ...
                  <input
                    ...
                    readOnly
                    className="w-full px-4 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-600 text-slate-900 dark:text-white cursor-not-allowed"
                  />
                  <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    Must equal duration — each member gets one payout
                  </p>
                </div>
              </div>
