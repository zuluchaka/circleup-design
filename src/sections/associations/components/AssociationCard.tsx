...
export function AssociationCard({ association, onView, onEdit }: AssociationCardProps) {
  ...
  const statusConfig = {
    active: { dot: 'bg-emerald-500', accent: 'from-emerald-500 to-teal-500', label: 'Active', pulse: true },
    draft: { dot: 'bg-amber-500', accent: 'from-amber-500 to-orange-500', label: 'Setting up', pulse: false },
    suspended: { dot: 'bg-red-500', accent: 'from-red-500 to-rose-500', label: 'Suspended', pulse: false },
    dissolved: { dot: 'bg-slate-400', accent: 'from-slate-400 to-slate-500', label: 'Dissolved', pulse: false },
  } as const

  const status = association.status && association.status in statusConfig
    ? statusConfig[association.status as keyof typeof statusConfig]
    : statusConfig.active
    ...
    >
      ...
      <div className="relative h-28 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex items-center gap-1.5 bg-black/40 backdrop-blur-sm rounded-full px-2 py-1">
          ...
          <span className="text-[11px] font-medium text-white/90">{status.label}</span>
        </div>
      ...
      <div className="pt-10 px-4 pb-4">
        ...
        <div className="flex gap-2 mt-3">
          ...
          {(association.myRole === 'president' || association.myRole === 'organizer') && (
            <button
              onClick={onEdit}
