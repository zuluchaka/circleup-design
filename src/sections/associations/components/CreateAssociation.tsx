...
import type { AssociationType, AssociationVisibility } from '@/../product/sections/associations/types'

export interface CreateAssociationProps {
  onSubmit?: (data: CreateAssociationData) => void | Promise<void>
  onSaveDraft?: (data: CreateAssociationData) => void | Promise<void>
  onCancel?: () => void
...
export function CreateAssociation({ onSubmit, onSaveDraft, onCancel, onStartMigration, error }: CreateAssociationProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSubmitError(e instanceof Error ? e.message : 'Failed to create association')
    } finally {
      setSubmitting(false)
    }
  }

  const basicsValid = formData.name.trim().length >= 3 && formData.description.trim().length >= 10
  ...
  const showSaveDraft = onSaveDraft && !isMethodStep && basicsValid
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 sm:p-8">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  aria-required="true"
                  aria-invalid={formData.name.length > 0 && formData.name.trim().length < 3}
                  className={`w-full px-4 py-3 text-slate-900 dark:text-white bg-slate-100 dark:bg-slate-800 border-0 rounded-xl focus:ring-2 focus:ring-indigo-500 ${
                    formData.name.length > 0 && formData.name.trim().length < 3 ? 'ring-2 ring-red-500' : ''
                  }`}
                />
                <div className="mt-2 flex justify-between">
                  <p className={`text-xs ${formData.name.length > 0 && formData.name.trim().length < 3 ? 'text-red-500' : 'text-slate-500'}`}>
                    {formData.name.length > 0 && formData.name.trim().length < 3 ? 'Name must be at least 3 characters' : 'Minimum 3 characters'}
                  </p>
                  <p className="text-xs text-slate-400">{formData.name.length}/100</p>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  maxLength={500}
                  aria-label="Association description"
                  aria-required="true"
                  aria-invalid={formData.description.length > 0 && formData.description.trim().length < 10}
                  className={`w-full px-4 py-3 text-slate-900 dark:text-white bg-slate-100 dark:bg-slate-800 border-0 rounded-xl focus:ring-2 focus:ring-indigo-500 resize-none ${
                    formData.description.length > 0 && formData.description.trim().length < 10 ? 'ring-2 ring-red-500' : ''
                  }`}
                />
                <div className="mt-2 flex justify-between">
                  <p className={`text-xs ${formData.description.length > 0 && formData.description.trim().length < 10 ? 'text-red-500' : 'text-slate-500'}`}>
                    {formData.description.length > 0 && formData.description.trim().length < 10 ? 'Description must be at least 10 characters' : 'Minimum 10 characters'}
                  </p>
          ...
          {currentStep === 'visibility' && (
            <div className="space-y-6">
              ...
              <div className="pt-4 border-t border-slate-200 dark:border-slate-800 space-y-4">
                <p className="text-sm text-slate-500 dark:text-slate-400">Optional contact information (can be added later in Settings)</p>
                <div>
                  <label className="block text-sm font-medium text-slate-900 dark:text-white mb-2">
                    Website
                  </label>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="info@association.ch"
                    className="w-full px-4 py-3 text-slate-900 dark:text-white bg-slate-100 dark:bg-slate-800 border-0 rounded-xl focus:ring-2 focus:ring-indigo-500"
                  />
                </div>
              </div>
          ...
          {currentStep === 'review' && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                {(formData.country || formData.website || formData.contactEmail) && (
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                    {formData.country && (
                      <div>
                        <h4 className="text-sm font-medium text-slate-500 dark:text-slate-500 mb-1">Country</h4>
                        <p className="text-slate-900 dark:text-white">{countryOptions.find((c) => c.value === formData.country)?.label}</p>
                      </div>
                    )}
                    {formData.website && (
                      <div>
                        <h4 className="text-sm font-medium text-slate-500 dark:text-slate-500 mb-1">Website</h4>
                        <p className="text-slate-900 dark:text-white truncate">{formData.website}</p>
                      </div>
                    )}
                    {formData.contactEmail && (
                      <div>
                        <h4 className="text-sm font-medium text-slate-500 dark:text-slate-500 mb-1">Contact Email</h4>
                        <p className="text-slate-900 dark:text-white">{formData.contactEmail}</p>
                      </div>
                    )}
                  </div>
                )}
          ...
          {/* Error */}
          {(error || submitError) && (
            <div className="mt-6 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl text-sm text-red-700 dark:text-red-400">
              {error || submitError}
            </div>
          )}
          ...
          <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800 flex justify-between">
            ...
            <div className="flex items-center gap-3">
              {showSaveDraft && currentStep === 'review' && (
                <button
                  onClick={handleSaveDraft}
                  disabled={savingDraft || submitting}
                  className="px-5 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl transition-colors"
                >
              ...
              {showSaveDraft && currentStep !== 'review' && (
                <button
                  onClick={handleSaveDraft}
                  disabled={savingDraft || submitting}
                  className="px-3 py-2.5 text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  {savingDraft ? 'Saving...' : 'Save as Draft'}
                </button>
              )}
              ...
              {currentStep === 'review' ? (
                <button
                  ...
                  className="px-6 py-2.5 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 dark:disabled:bg-indigo-800 disabled:cursor-not-allowed rounded-xl transition-colors shadow-lg shadow-indigo-500/25"
                >
